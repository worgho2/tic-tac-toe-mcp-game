...
- `lobby/types.ts`: constants (`PRESENCE_TTL_MS = 30_000`, `INVITE_TTL_MS = 60_000`, `REMATCH_DELAY_MS = 3_000`, `MAX_NAME = 24`) and the public view types (`PlayerView`, `GameView`, `InviteView`, `LobbyEvent`). Re-exported from `lobby.ts`.
...
- `tools/handlers.ts`: one pure function per tool. Uniform contract: `lobby.reconnect(playerId)` (touch, or re-create a swept id in the name phase so a reloaded widget can register again) → `lobby.sweep()` → mutate → return `viewFor(playerId)` with the transient `error` merged in. Every reply duplicates the payload in both `structuredContent` and a JSON `content[0].text` block. New tools should follow this exact shape.
